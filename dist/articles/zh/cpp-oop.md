...
class "Base::vtable" as B_VTBL #FFF9C4 {
  + slot[0]: 0x0040A100 → Base::func1()
  + slot[1]: 0x0040A108 → Base::func2()
}

class "Derived::vtable" as D_VTBL #FFECB3 {
...
note bottom of D_VTBL
  **slot[0] 被重写指向 Derived::func1**
end note

== **第三部分：对象内存布局与 vptr** ==

object "Base 对象\n(sizeof=12B)" as BASE_OBJ #E8F5E9 {
  {field} vptr ──────────────►
  {field} data: int = 10
}

object "Derived 对象\n(sizeof=16B)" as DERIVED_OBJ #E3F2FD {
  {field} vptr ──────────────►
  {field} data: int = 10
  {field} extraData: int = 20
}

BASE_OBJ -right-> B_VTBL : vptr 指向
DERIVED_OBJ -right-> D_VTBL : vptr 指向
...
== **第四部分：动态绑定流程 ptr->func1()** ==

actor "程序员" as DEV #FFF3E0
participant "Base* ptr" as PTR #B3E5FC
participant "vptr\n(对象头部)" as VPTR #C8E6C9
participant "Derived::vtable\n(类数据)" as VTBL #FFF9C4
participant "Derived::func1()\n(实际函数)" as FUNC #FCE4EC

DEV -> PTR : ptr = new Derived()\nptr->func1() ← 调用哪个?
...
note over VTBL #FFFFFF
  new Derived() → vptr 指向\nDerived::vtable (非 Base::vtable)
end note

VTBL -> VTBL : 3. 查找 slot[0]\nDerived::vtable[0] = 0x0040B200

note over VTBL #FFFFFF
  **关键: slot[0] 指向 Derived::func1**\n**不是 Base::func1!**
end note

VTBL -> FUNC : 4. call 0x0040B200\n跳转执行
FUNC --> DEV : 5. 执行 Derived::func1()\n实现多态!

== **第五部分：构造/析构过程中 vptr 的变化** ==

participant "Derived 对象" as OBJ #E8F5E9
...
note right of BASE_CTOR #FFFFFF
  **Base 构造阶段:**
  调用 func1() → Base 版本\n(Derived 部分未构造!)
end note

DERIVED_CTOR -> DERIVED_CTOR : 设置 vptr → Derived::vtable
note right of DERIVED_CTOR #FFFFFF
  **Derived 构造阶段:**
...
legend center
  | 核心概念 | 说明 |
  |------|------|
  | vptr | 对象头部隐藏指针，指向 vtable |
  | vtable | 类级别数组，存储虚函数地址 |
  | slot[i] | 第 i 个虚函数入口 |
  | 动态绑定 | 通过 vptr 运行时查找实际函数 |
  | 重写效果 | Derived::vtable slot[i] 指向新函数 |
endlegend
