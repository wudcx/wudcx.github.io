...
package "编译后 - vtable 生成" {
  class "Base::vtable" as B_VTBL #FFF3E0 {
    + slot[0] → Base::func1 地址
    + slot[1] → Base::func2 地址
  }

  class "Derived::vtable" as D_VTBL #FCE4EC {
    + slot[0] → Derived::func1 地址  // 重写
    + slot[1] → Base::func2 地址     // 继承
  }
...
note "每个有虚函数的类都有自己的 vtable" as N1 #FFCCCC
...
title **多态实现原理 - 对象内存布局与 vptr

together {
  object "Base 对象" as BASE_OBJ #E8F5E9 {
    {static} vptr ──────────────►
    data: int = 10
  }

  object "Derived 对象" as DERIVED_OBJ #E3F2FD {
    {static} vptr ──────────────►
    data: int = 10
    extraData: int = 20
  }
...
together {
  ...
  class "Derived::vtable" as D_VTBL #FCE4EC {
    + slot[0]: 0x0040B200 → Derived::func1()  ← 重写!
    + slot[1]: 0x0040A108 → Base::func2()      ← 继承
  }
}

BASE_OBJ -right-> B_VTBL : new Base() 后\nvptr 指向
DERIVED_OBJ -right-> D_VTBL : new Derived() 后\nvptr 指向
...
legend right
  | 关键点 | 说明 |
  |------|------|
  | vptr | 对象首字节，指向类的 vtable |
  | vtable | 类共享，存储虚函数地址数组 |
  | slot[i] | 第 i 个虚函数的入口地址 |
endlegend
...
title **多态实现原理 - 动态绑定完整流程**

actor "程序员" as DEV #FFF3E0
participant "Base* ptr" as PTR #E8F5E9
participant "对象内存\nvptr" as VPTR #E3F2FD
participant "vtable\n(类数据)" as VTBL #FFF3E0
participant "实际函数" as FUNC #FCE4EC
 ...
 PTR     ->  VPTR    : 1. 读取 ptr->vptr\n(对象头部隐藏指针)
 activate VPTR

 VPTR    ->  VTBL     : 2. 通过 vptr 找到 vtable\n(哪个类的?)
 activate VTBL

 note over VTBL #FFFFFF
   new Derived() 创建的是 Derived 对象
   所以 vptr 指向 Derived::vtable
 end note

 VTBL    ->  VTBL     : 3. 查找 slot[0]\n(第0个虚函数)
 note over VTBL #FFFFFF
   Derived::vtable[0] = Derived::func1 地址\n(被重写了!)
 end note

 VTBL    ->  FUNC     : 4. 跳转到 0x0040B200\nDerived::func1()
 activate FUNC

 FUNC    -->  DEV      : 5. 执行 Derived::func1()\n而不是 Base::func1()!
 deactivate FUNC
...
| 步骤 | 动作 | 关键指令 |
|------|------|---------|
| 1 | 读取 vptr | mov eax, [ptr] |
| 2 | 找 vtable | eax = [eax] |
| 3 | 查 slot | eax = [eax + slot*4] |
| 4 | 跳转 | call [eax] |
endlegend
...
title **多态实现原理 - 构造过程中 vptr 的变化**

participant "Derived 对象\n内存布局" as OBJ #E8F5E9
participant "Base 构造函数" as BASE_CTOR #E3F2FD
participant "Derived 构造函数" as DERIVED_CTOR #FFF3E0

note over OBJ #FFFFFF
  初始状态: 对象内存分配\nvptr 未初始化
end note

BASE_CTOR -> BASE_CTOR : 设置 vptr → Base::vtable
note right of BASE_CTOR #E8F5E9
  **Base 构造阶段**
  调用的是 Base::func() → Base 版本
  (Derived 部分还未构造!)
end note

DERIVED_CTOR -> DERIVED_CTOR : 设置 vptr → Derived::vtable
note right of DERIVED_CTOR #E3F2FD
  **Derived 构造阶段**
  vptr 更新为指向 Derived::vtable
  现在调用的是 Derived::func() → Derived 版本
end note
...
legend right
  | 阶段 | vptr 指向 | 虚函数调用目标 |
  |------|----------|--------------|
  | Base 构造中 | Base::vtable | Base 版本 |
  | Derived 构造中 | Derived::vtable | Derived 版本 |
  | 析构 Derived | Derived::vtable | Derived 版本 |
  | 析构 Base | Base::vtable | Base 版本 |
endlegend
...
通过图示理解 vtable 与 vptr 的创建和初始化过程：
